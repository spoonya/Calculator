class Calculator {
  ...
  numSeparator(num) {
    ...
    if (isNaN(intDigits)) {
      ...
      intDisplay = intDigits.toLocaleString('ru', {
...
operationBtnSqrt.addEventListener('click', () => {
  if (calc.curOperand < 0) {
    calc.curOperandTxtEl.innerText = 'Wrong argument!';
    calc.curOperand = '';
  } else if (!isNaN(calc.curOperand) && calc.curOperand !== ''){
...
equalsBtn.addEventListener('click', () => {
  if (calc.curOperand == 0 && calc.operation === '/') {
    calc.curOperandTxtEl.innerText = 'Division by zero!';
    calc.curOperand = '';
    calc.readyToReset = true;
  } else {
    calc.compute();
